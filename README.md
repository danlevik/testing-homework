...
Пожалуйста, выполняйте тесты через hermione gui (все тесты написаны через hermione) и если они падают, пробуйте их ретраить по отдельности друг от друга (в частности тесты, которые изменяют количество товаров в корзине), они могут накладываться друг на друга.
