...
Тесты запускать через npm run test:e2e и потом смотреть результат в npx hermione gui. (Все тесты написаны с помощью hermione)

В конфиге по умолчанию стоит 3 ретрая

Если тесты упали, запустите их по отдельности еще раз через hermione gui (в частности тесты, которые изменяют количество товаров в корзине), они могут накладываться друг на друга.

Для запуска приложения с багом на Windows можно использовать команду

```sh
$env:BUG_ID = '0'; npm run start
```

В данном случае BUG_ID = '0' - отсутствие багов.
